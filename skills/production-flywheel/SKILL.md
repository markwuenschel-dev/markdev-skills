...
**Failure interrupt:** if work is broken, red, flaky, slow, or unexplained, pause and run `/diagnosing-bugs` (red-capable loop, minimized failure, regression test at the right seam, cleanup), then resume.
...
- **the queue is empty** → print a final per-item summary (each item's branch/PR/status) and stop, or
