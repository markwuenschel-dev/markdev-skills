/* ledger-verify.js — self-verifying scoring ledger for the shared candidate
 * spine used by repository health, architecture, and integrity reports.
 *
...
function verifyLedger(ledger, opts) {
  ...
  if (ledger.spine_version !== 3) {
    push("spine-version", null, "ledger.spine_version must be 3 (got " + JSON.stringify(ledger.spine_version) + ")");
  }
  ...
  ledger.candidates.forEach((c, i) => {
    ...
    // Lifecycle is separate from v3 scheduling status. New reports carry it;
    // historical ledgers without it remain readable for continuity migration.
    const lifecycle = c && c.candidate_lifecycle;
    if (lifecycle != null) {
      const states = ["discovered", "awaiting-selection", "designing", "ready", "active", "verifying", "completed", "blocked", "rejected", "deferred", "superseded", "out-of-scope"];
      if (!lifecycle || lifecycle.candidate_id !== id || states.indexOf(lifecycle.lifecycle_status) < 0 ||
          !lifecycle.current_owner || typeof lifecycle.current_owner.skill !== "string" || typeof lifecycle.current_owner.stage !== "string" ||
          !Array.isArray(lifecycle.history) || !lifecycle.completion || !lifecycle.resolution) {
        push("lifecycle-shape", ref, ref + ": candidate_lifecycle is incomplete or does not preserve candidate_id");
      } else if (lifecycle.lifecycle_status === "completed" &&
          (!lifecycle.completion.completed_at || !Array.isArray(lifecycle.completion.verification_refs) || !lifecycle.completion.verification_refs.length)) {
        push("lifecycle-completion", ref, ref + ": completed lifecycle requires completed_at and verification_refs");
      } else if (lifecycle.lifecycle_status === "superseded" && !lifecycle.resolution.superseded_by) {
        push("lifecycle-superseded", ref, ref + ": superseded lifecycle requires resolution.superseded_by");
      } else if (["rejected", "deferred", "out-of-scope"].indexOf(lifecycle.lifecycle_status) >= 0 && !lifecycle.resolution.reason) {
        push("lifecycle-resolution", ref, ref + ": terminal lifecycle requires resolution.reason");
      }
    }
