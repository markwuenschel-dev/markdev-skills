...
Candidate selection is two phases, and every consumer of a candidate ledger — the recommended queue, auto mode, any production-flywheel handoff, and any "top recommendation" that claims to represent execution priority — uses them identically. This section is the single authoritative eligibility rule and comparator: other files restate it only verbatim with a reference here, and `ledger-verify.js` in this shared package is its executable form.
