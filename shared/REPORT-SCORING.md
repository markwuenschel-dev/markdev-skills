...
**Human-decision taxonomy:** `human_decision_risk`, `blocked_by`, and the *Human-decision blockers* report section are named against `shared/HUMAN-DECISIONS.md` (this repo). The optional external skill `connected-impact-sweep` may expand edit-coherence detail when installed — see DEPENDENCIES.md — without forking category names.
...
rollup:
  ...
  blocked_by:            # human-decision forks, named against shared/HUMAN-DECISIONS.md
dedup:
  duplicate_group:
  merged_from:
```
...
- **Human-decision risk** — how likely the fix requires public-contract, migration, policy, architecture, deletion, or numerical-correctness judgment (the forks under shared/HUMAN-DECISIONS.md).
...
8. **Human-decision blockers** — each named against shared/HUMAN-DECISIONS.md
...
13. **Recommended queue** — priority order, with blocked candidates listed as decisions-needed rather than dropped
...
2. *By priority band* — three big Space-Grotesk counts: High ≥12 (ink), Mid 6–11 (ink2), Low ≤5 (faint). Bands are cut on the `priority_score`, so they hold for any repo.
...
Drive the score matrix, top-candidates table, lane scorecards, triage strip, and filter chips from **one array in the logic** (one row object per candidate: `id, title, action, the eight 1–5 scores, priority_score, blocked`), not hand-written rows — it keeps the priority-heat mapping, summary counts, and blocked markers consistent and makes re-scoring a data edit. Hand-author only the prose sections and the per-candidate `<details>` bodies (their evidence text is unique).
